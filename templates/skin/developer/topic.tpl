...
<div class="topic" id="topic_id_{$oTopic->getId()}">
<div class="idea">
...
<div class="idea-top">
		...
		<div class="status" id="status_{$oTopic->getId()}" {if !$oTopic->getStatus()}style="display:none;"{/if}><img src="{cfg name="path.static.skin"}/images/{$oTopic->getStatus()}.png" /></div>
	<h2 class="title">
	...
	<ul class="actions">									
		...
		{/if}
		{if $oUserCurrent and ($oUserCurrent->isAdministrator() or $oBlog->getUserIsAdministrator() or $oBlog->getOwnerId()==$oUserCurrent->getId())}
			<li><a href="#" title="Odpowiedź" onclick="toggleAnsBlock('answer_block_{$oTopic->getId()}',this);return false;" class="answer">Odpowiedź</a></li>
		{/if}
	</ul>
	{if $oUserCurrent and ($oUserCurrent->isAdministrator() or $oBlog->getUserIsAdministrator() or $oBlog->getOwnerId()==$oUserCurrent->getId())}
	...
	<form name="answer_form_{$oTopic->getId()}" id="answer_form_{$oTopic->getId()}" method="POST" action="" onsubmit="return false;" enctype="multipart/form-data">
		...
		<select style="width: 120px;" name="topic_status">  
					...
					<option value="2" {if $oTopic->getStatus() == 'started'}selected="selected"{/if}>Wykonuje się</option>    

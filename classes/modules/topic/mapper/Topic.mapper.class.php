...
class ModuleTopic_MapperTopic extends Mapper {	
	...
	public function AddTopic(ModuleTopic_EntityTopic $oTopic) {
		...
		if ($iId=$this->oDb->query($sql,$oTopic->getBlogId(),$oTopic->getUserId(),$oTopic->getType(),$oTopic->getTitle(),
			$oTopic->getTags(),$oTopic->getDateAdd(),$oTopic->getUserIp(),$oTopic->getPublish(),$oTopic->getPublishDraft(),$oTopic->getPublishIndex(),$oTopic->getCutText(),$oTopic->getForbidComment(),$oTopic->getTextHash(),$oTopic->getTopicCat())) 
		{
			$oTopic->setId($iId);
	...
	/**
	 *  Ищем в базе топики по заголовку
	 *
	 * @param  string $sTopic_title
	 * @return unknown
	 */
	public function GetTopicsByTopicTitle($stroka,$sBlogId) {
		$sFind = strtoupper($stroka); 
		$sFind = addslashes($sFind); 
		$sql = "SELECT 
					t.*,
					tc.*							 
				FROM 
					...
					JOIN  ".Config::Get('db.table.topic_content')." AS tc ON t.topic_id=tc.topic_id				
				WHERE 
					upper(t.topic_title) LIKE '%$sFind%' AND t.topic_publish = 1 AND t.blog_id = ?d								
				ORDER BY t.topic_title";
		$aTopics=array();
		if ($aRows=$this->oDb->select($sql,$sBlogId)) {
			foreach ($aRows as $aTopic) {
				$aTopics[]=Engine::GetEntity('Topic',$aTopic);
			}
		}		
	...
	public function UpdateTopicAnswer(ModuleTopic_EntityTopic $oTopic) {		
		$sql = "UPDATE ".Config::Get('db.table.topic_content')." 
			...
			WHERE
				topic_id = ?d
		";			
		if ($this->oDb->query($sql,$oTopic->getAnswerText(),$oTopic->getAnswerData(),$oTopic->getAnswerAutor(),$oTopic->getStatus(),$oTopic->getId())) {
			return true;
		}		
		return false;
	}

...
class ModuleTopic extends Module {		
	...
	/**
	 * Обновляет ответ к топику
	 *
	 * @param ModuleTopic_EntityTopic $oTopic
	 * @return unknown
	 */
	public function UpdateTopicAnswer(ModuleTopic_EntityTopic $oTopic) {
		if ($this->oMapperTopic->UpdateTopicAnswer($oTopic)) {	
			//чистим зависимые кеши			
			$this->Cache_Clean(Zend_Cache::CLEANING_MODE_MATCHING_TAG,array('topic_update',"topic_update_user_{$oTopic->getUserId()}","topic_update_blog_{$oTopic->getBlogId()}"));
			$this->Cache_Delete("topic_{$oTopic->getId()}");
			return true;
	...
	/**
	 * Сравнивает рэйтинг, для сортировки
	 *
	 */
	public static function cmp($a,$b){
		if ($a->getCountVote() == $b->getCountVote()) {
        return 0;
		}
		return ($a->getCountVote() < $b->getCountVote()) ? 1 : -1;
	}
	/**
	...
	/**
	 * Получить список топиков по заголовку
	 *
	...
	public function GetTopicsByTopicTitle($stroka,$sBlogId) {
		$aTopics=array();	
		if (false === ($data = $this->Cache_Get("topic_title_{$stroka}"))) {			
			$data = $this->oMapperTopic->GetTopicsByTopicTitle($stroka,$sBlogId);
			foreach ($data as $oTopic) {
				$aTopics[$oTopic->getId()]=$oTopic;
			}
			$this->Cache_Set($aTopics, "topic_title_{$stroka}", array("topics_titles"), 60*60*24*1);
			return $aTopics;
		}		
		return $data;
	}

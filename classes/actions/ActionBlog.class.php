...
class ActionBlog extends Action {
	...
	protected function RegisterEvent() {			
		...
		$this->AddEvent('widzet','EventWidzetBlog');
		...
		$this->AddEvent('my','EventShowUserBlogs');
	 ...
	 * @return unknown
	 */
	protected function EventWidzetBlog() {		
		/**
		 * Меню
		 */		
		$this->sMenuItemSelect='widzet';
		$this->sMenuSubItemSelect='';
		/**
		 * Проверяем передан ли в УРЛе номер блога
		 */
		...
		$this->Viewer_AddHtmlTitle("Widżet");
		
		$this->Viewer_Assign('oBlogEdit',$oBlog);
		$this->Viewer_Assign('oBlogTitle',$oBlog->getTitle());
		$this->Viewer_Assign('oBlogUrl',$oBlog->getUrl());
		/**
		 * Устанавливаем шалон для вывода
		 */		
		$this->SetTemplateAction('widzet');
	}
	/**
	 * Управление пользователями блога
	 *
	...
	protected function SubmitComment() {
		...
		if (!($oTopic=$this->Topic_GetTopicById(getRequest('cmt_target_id')))) {
			 $this->Message_AddError('1','Ошибка');
			return;
		}
		/**
		...
		if (!$oTopic->getPublish() and $this->oUserCurrent->getId()!=$oTopic->getUserId() and !$this->oUserCurrent->isAdministrator()) {
			 $this->Message_AddError('2','Ошибка');
			return;
		}
		if ($this->oUserCurrent->getId() == '2') {
		/**
		...
		*/
		...
		if (!func_check($sParentId,'id')) {			
			 $this->Message_AddError('3','Ошибка');
			return;
		}
		$oCommentParent=null;
		if ($sParentId!=0) {
			...
			*/
			if (!($oCommentParent=$this->Comment_GetCommentById($sParentId))) {				
				 $this->Message_AddError('4','Ошибка');
				return;
			}
			/**
			...
			*/
			if ($oCommentParent->getTargetId()!=$oTopic->getId()) {
				 $this->Message_AddError('5','Ошибка');
				return;
			}
		} else {
			/**
	...
	protected function EventShowUserBlogs() {
		if($this->oUserCurrent){
		$sMenuHeadItemSelect='my';
		/**
		 * Получаем список блогов
		 */
		$aBlogs=$this->Blog_GetBlogsRatingSelf($this->oUserCurrent->getId(),1000);
		/**
		 * Загружаем переменные в шаблон
		 */							
		$this->Viewer_Assign("aBlogs",$aBlogs);
		$this->Viewer_AddHtmlTitle("Moje projekty");
		/**
		 * Устанавливаем шаблон вывода
		 */
		$this->SetTemplateAction('my');		
		}
		else
		{
			return parent::EventNotFound();	

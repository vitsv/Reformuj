...
class ActionTopic extends Action {
	...
	public function Init() {
		...
		if (!$this->User_IsAuthorization()) {
			$this->oUserCurrent = $this->User_GetUserById(2);
		}
		else
		{
	...
	protected function RegisterEvent() {		
		...
		$this->AddEvent('ajaxaddtopic','AjaxAddTopic');
		$this->AddEvent('ajaxaddanswer','AjaxAddAnswer');
	...
	/**
	 * Добавление ответа для топика аяксом
	 *
	 * @return unknown
	 */
	
	protected function AjaxAddAnswer() {
	 ...
	 * @return unknown
	 */
	protected function SubmitAnswer() {
		$this->Security_ValidateSendForm();
		/**
		 * Получаем номер топика из УРЛ и проверяем существует ли он
		 */
		if(isset($_REQUEST['topic_id'])){
		$sTopicId=getRequest('topic_id');
		if (!($oTopic=$this->Topic_GetTopicById($sTopicId))) {
		 ...
		 */
		if (!func_check(getRequest('answer_text',null,'post'),'text',2,Config::Get('module.topic.max_length'))) {
		...
		if (!$this->ACL_IsAllowEditTopic($oTopic,$this->oUserCurrent)) {
		 	...
		 	* Обрабатываем отправку формы
			*/		
			$oTopic->setAnswerText(getRequest('answer_text'));
			$oTopic->setAnswerData(date("Y-m-d H:i:s"));
			$oTopic->setAnswerAutor($this->oUserCurrent->getId());
			$oTopic->setStatus(getRequest('topic_status'));

			/**
			 * Сохраняем топик
			 */
			if ($this->Topic_UpdateTopicAnswer($oTopic)) {
				Router::Location($oTopic->getBlog()->getUrlFull());
			} else {
		...
		{
			$this->Message_AddErrorSingle('Nie znaleziono wpisu!',$this->Lang_Get('error'));
			return false;
		}
	...
	/**
	 * Добавление топика аяксом
	 *
	 * @return unknown
	 */
	
	protected function AjaxAddTopic() {
		$this->Viewer_SetResponseAjax();
		$this->SubmitAdd();
	}
	...
	protected function SubmitAdd() {
		...
		/*if (!isPost('submit_topic_publish') and !isPost('submit_topic_save')) {
			$this->Message_AddError('Nic nie wysłano!','Błąd');
			return false;
		}*/	
		/**
		...
		if (!$this->checkTopicFields()) {
			$this->Message_AddError('Nieporawne dane!','Błąd');
			return false;	
		}		
		/**
		 * Определяем в какой блог делаем запись
		...
		$oTopic->setTopicCat(getRequest('topic_cat'));
		/**
		...
		if ($this->Topic_AddTopic($oTopic)) {
			...
			$this->Viewer_AssignAjax('sTopicId',$oTopic->getId());
